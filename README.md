...
| Getting the cursor back | ✅ **Escape toggles the pointer.** Press it once and the cursor is yours — for a dialog the game put in the middle of the screen, or a camera that has stuck. Press it again and the game has it back. Escape reaches Roblox as well, so the menu it opens and the cursor it frees move together. Leaving first person hands the lock back normally, and `CORDIAL_NO_POINTER_LOCK=1` turns capture off for a whole session |
...
| `CORDIAL_NO_POINTER_LOCK=1` | never capture the cursor; Escape toggles it case by case |
